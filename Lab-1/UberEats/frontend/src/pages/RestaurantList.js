...
const RestaurantList = () => {
  ...
  const handleFavouriteToggle = async (restaurantId) => {
    if (!isLoggedIn) {
      alert('Please login to favourite restaurants');
      return;
    }
    try {
      ...
      if (response.ok) {
        setFavourites((prev) => ({
          ...prev,
          [restaurantId]: !isFavourited,
        }));
      } else {
        const errorData = await response.json();
        alert(errorData.error || 'Failed to update favourites');
      }
    } catch (error) {
            ...
            {restaurants.map((restaurant) => (
              <div key={restaurant.id} className="restaurant-card-vertical">
                ...
                <div className="restaurant-details">
                  ...
                  <p className="description">{restaurant.description}</p> {/* Display description dynamically */}
